...
def process_img(orgin_img):
    ...
    ret, thresh = cv2.threshold(process_img, 120, 255, cv2.THRESH_BINARY)
    ...
    vert = np.array([[10, 500], [10, 300], [300, 200], [500, 200], [800, 300], [800, 500]], np.int32)
    process_img = roi(process_img, [vert])

    lines = cv2.HoughLinesP(process_img, 1, np.pi/2, 180, 20, 15)   # 检测图中满足要求的直线并返回坐标
    try:
        l1, l2 = draw_lanes(process_img, lines)   # 根据坐标画线
    ...
    try:
        ...
        pass
    return process_img, orgin_img, thresh
...
while 1:
    ...
    gray_screen, origin_screen, thresh = process_img(screen)
    # 转为numpy以便imshow，并且由于color出错需要改变颜色

    cv2.imshow('window', gray_screen)
    cv2.imshow('window2', cv2.cvtColor(origin_screen, cv2.COLOR_BGR2RGB))
